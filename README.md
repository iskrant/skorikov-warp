...
[PRIVATE.md](PRIVATE.md): запуск `./scripts/codex` подготавливает окружение
и устанавливает проверку приватного архива перед commit.

Одностраничная галерея работ художника Скорикова Игоря Андреевича, созданная на базе Hugo для размещения на GitHub Pages.

Сборка релиза: `sh scripts/build-site --gc --minify`. В исходнике HTML сохраняется
комментарий `release: commit=… built_at=…` с SHA исходников и временем сборки UTC.
Незакоммиченные изменения помечаются `-dirty`; обычный запуск Hugo без скрипта
помечается `unversioned`.
